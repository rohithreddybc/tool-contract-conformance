...
The paper's central deliverable is not a violation count. It is a trace: given a confirmed defect, which task verdicts could have been computed on state that defect corrupted. `analysis/score_at_risk.py` performs the trace as three static steps, no model anywhere in the pipeline, over the 1,208 rows written to `report/score_at_risk.jsonl`. A `--verify` flag re-derives every row from the same pinned inputs and diffs the result against the file on disk, so the trace is checked before it is trusted, not merely reproducible in principle.
...
These bounds do not indict the taxonomy on their own. `experiments/decompose_closed_world_misses.py` traces the cause of all 33 real-tool misses: 29 are `probe_corpus_unreachable` (the frozen corpus never drove the mutated path), 2 are a genuine `clause_gap`, 1 a `probe_gap`, 1 `target_not_found`. Per operator, M-PRECOND's 6 misses and M-INVAR's 8 are all unreachable; M-IGNARG's 12 are 9 unreachable, 2 clause gaps, 1 target-not-found; M-PARTIAL's 7 are 6 unreachable, 1 probe gap. M-INVAR, decomposed by liveness screening only (`operators_liveness_only`) not the full three-way classifier, has a 0.000 that reads as "no probe ever reached a mutated invariant path," not "the checker cannot detect invariant breaks" — only 2 of 33 misses are a taxonomy gap the checker itself owns. Two questions stay open: whether closed-world mutants were screened for behavioural liveness before entering this denominator, and whether the six watchdog timeouts (M-PHANTOM 2, M-IGNARG 4) are exchangeable with misses for the bounds above. The bounds are unchanged; only what a miss means changes. M-RESET has no real-tool site to draw at all: no tool-implementation file in any of the three real benchmarks defines a state-restoring function, so the operator's own site pool is empty before any exclusion is applied, and the honest report is "no data," not a rate of zero. Biconditional success, the mechanism M-PHANTOM's detector depends on, is opt-in per contract and is adopted on 18 of 19 tau2-bench contracts, 1 of 7 AgentDojo contracts, and 0 of 5 MM-ToolSandbox contracts; where it is not adopted, Phantom Effect cannot fire on that tool at all, a second, more visible reason M-PHANTOM's own recall sits at 0.359 rather than higher.
...
**The equivalent real-benchmark run produced no usable number at all, and the absence is reported rather than hidden.** cosmic-ray scored 465 mutations across 2,325 (mutation, tool) pairs against tau2-telecom's five non-anchor tools; zero were behaviourally live, so the escape-rate denominator is zero and the rate is undefined. The cause is the probe corpus, not the checker: the pre-registered corpus's third component, calls recorded from real executions, has no artifact anywhere in this project, and the two components that do exist are type-generic placeholders that fail existence checks on tau2's entity-keyed tools before any mutation could show through them. A zero denominator on this arm is not evidence that the checker covers tau2-telecom well; it is evidence that nothing exercised it. The same missing artifact — no production caller ever supplies a recorded call — also explains the `reserve_car_rental` agreement flip (every probe died in `datetime.fromisoformat()` before reaching effect code) and 29 of §VII's 33 real-tool misses, making a recorded-real-calls corpus this project's highest-value future work rather than a generic limitation.

**Sample sizes fall short of the pre-registration for four of six mutation operators, and the shortfall is structural, not a scoring choice.** The plan committed to roughly 25 real-tool sites per class. M-PRECOND and M-IGNARG met it. M-PHANTOM's entire real-tool corpus is 35 sites, one per tool, and the first freeze had already drawn 25 of them before any refreeze; M-PARTIAL's pool of 36 left only 11 unused; M-INVAR and M-RESET, at pools of 10 and 5, had nothing left to draw and fell back to rescoring the first freeze's own sites, the one documented exception to the project's no-reuse rule. Achieved N is reported beside the pre-registered target in every case, never silently absorbed into a rate.

**M-RESET has no real-tool site at all.** No tool-implementation file in any of the three real benchmarks defines a state-restoring function; reset logic lives in the adapter layer, which reconstructs the environment wholesale rather than repairing it in place. Only the synthetic toy domain colocates reset with its tools, deliberately, so the operator has anything to enumerate. The honest report for M-RESET on real tools is "no data," not a recall of zero, and Reset Leak's field-observed count anywhere in this project's findings is zero.

**The subprocess boundary absorbs exceptions the checker never sees, and whether that matters is unverified.** A real bug on the toy domain during the closed-world v2 run showed the mechanism directly: an adapter's `invoke()` caught only its own declared error type, so a probe built to violate a deleted guard drove the tool past the missing check into code that assumed the check held, and the resulting raw exception aborted the mutant instead of registering as a violation. Toy is a debug domain and never enters the headline, but the same subprocess boundary sits between the checker and every real adapter, and it swallows arbitrary exceptions before the harness can classify them. Whether an absorbed crash on a real tool would score as a detected violation or a silent non-detection has not been checked. That bounds what every real-tools recall number in §VII means, and the bound is left unresolved rather than argued away.
...
**One refreeze cycle is disclosed rather than folded into the final numbers.** A mutation-corpus sweep against `checker-freeze-v1` found four taxonomy claims the checker had been carrying as closed with no code behind them: Reset Leak and Invariant Break had no code path that ever produced their verdict, 35 shipped frame clauses were never evaluated against a real snapshot pair, and a hand-written routing table meant most tau2 contracts were never dynamically exercised at all. All four were fixed and the checker was refrozen as `checker-freeze-v2` before any number in §VII was scored. A framework that reports the sweep that found its own holes earns more trust than one presented as complete from the start, and the cost of finding them — one refreeze cycle rather than zero — is stated plainly rather than absorbed into the total.
