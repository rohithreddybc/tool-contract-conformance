...
Across four shipped benchmarks, the ledger's twelve VIOLATES clause rows collapse to eight confirmed finding instances, populating seven benchmark-class cells; tau2-bench's unadjudicated `suspend_line` candidate (below) adds an eighth, matching Table III's own Classes total (caption and footnote state what that column counts). The compressed count leads deliberately: this project's reporting rule counts unique defect classes per benchmark, because eight bugs in one copied helper are one bug. Four of the eight cells cannot carry a headline on our own rules: tau2-bench's Partial Effect is maintainer-annotated (a `logger.warning` the agent never sees), not agent-visible; MedAgentBench's Ungrounded Oracle is a property of the evaluator, not one of the six tool-layer classes; AgentDojo's Phantom Effect cell is real in the contract but unconfirmed in practice, since its biconditional re-tag needs the `recurring` and `amount` effects to violate on the same call and the banking fixture never does that; and `suspend_line` is itself an unadjudicated classification, not one of the eight confirmed instances. All four are reported in full, labelled, and excluded, giving **eight cells, of which four are headline-eligible**. Four of the eight confirmed instances are Ignored Argument. Every finding was verified by direct source reading at a pinned commit, independently of whatever process (human or the checker itself) first surfaced it, and each carries a clean-clone reproduction command in the artifact. Per-benchmark totals appear in Table III [N1: report/render.py ← findings.jsonl].
...
**MedAgentBench (Findings 1 and 4) is the paper's central chain.** Finding 1 is the no-op POST branch quoted in §I. Finding 4 is what makes it undetectable from inside the benchmark: the grading module reconstructs each write from the conversation transcript, admitting evidence only when followed by the fabricated success string of Finding 1 (§II). No grader ever issues a FHIR read to verify the write. Patching the tool to perform real writes would change no score, because no grader observes the state a patch would repair, and that is why §VII attempts no A/B experiment on this benchmark. Finding 4's evidence file, `refsol.py`, is not distributed with the repository; MedAgentBench's own README directs a reader to an unhashed, unversioned Stanford Medicine Box share for it. We pin a copy in the artifact at its SHA-256 hash (`repro/env/refsol.py`), so a reader can verify §IV's 60/90/150/0 write-oracle classification against that hash rather than trusting our transcription.

Two classes recur across the other three benchmarks: an interface tells the agent a call has an effect, and the implementation either drops it silently (Ignored Argument, four of eight instances) or reports success regardless (Phantom Effect, stacked on Finding 5). A single-layer read of the tool or the interface alone finds nothing wrong, because each layer is internally consistent with itself.

All four benchmarks are cited at the audited commits [medagentbench25], [tau2bench25], [agentdojo24], [mmtoolsandbox26]; instance-level evidence is in the findings ledger, reproducible from clean clones with the commands shipped in the artifact.
