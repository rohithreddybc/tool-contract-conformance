...
4. **Open-world-validated detection**: precision and recall against both taxonomy-shaped and off-taxonomy injected defects, under a pre-registered plan (§VII).
...
We did not invent design by contract, contract inference, or benchmark auditing: §II concedes the checking technique and rests the paper's novelty on the target and the consequence for published scores.

One result shapes the whole argument: MedAgentBench's grader admits evidence only when followed by the literal success string its own defective tool fabricates, so the tool and the grader meant to catch it are mutually consistent, and five prior audits reported nothing (§II, §VIII).
...
Contract authoring is measured, not trusted, but the measurement is of a narrower thing than independent replication. Both annotator sets, A and B, are agent sessions on the same underlying model family, and only B authored blind. A already knew which tools carried confirmed findings; what follows is evidence of **stability of sentence-to-predicate translation under one-way blindness**, not of independent human judgment, and the paper claims no more than that. The two sets are compared extensionally against a shared, pre-committed probe corpus across six tools, over comparable clause pairs on successful calls only. The blind stratum agrees on 98 of 98 probe comparisons across 16 clause pairs, a span covering four of the five blind tools; the fifth, `venmo_social`, contributes zero comparable pairs, because the blind annotator segmented the multiplexed tool into disjoint dispatch branches and never declared Finding 7's own anchor `arg.sort_by`. That anchor is incomparable, not agreeing, itself a segmentation result about multiplexed tools. Four tools (`refuel_data`, `cancel_reservation`, `update_scheduled_transaction`, `venmo_social`) needed hand-built witness probes, disclosed before any outcome was inspected, because the frozen corpus could not reach them. A sixth tool, `cancel_reservation`, forms a contaminated stratum (Annotator B had read a worked example of this exact tool beforehand) and is reported separately, never pooled with the blind result. One finding flips within the blind stratum: `reserve_car_rental`'s `arg.end_time` is VIOLATES under A and UNTESTABLE under B, because A's contract declares a `probe_values` hint pinning the argument to parseable ISO strings and B's does not, so every probe B's contract can build fails inside `datetime.fromisoformat()` before reaching the tool's effect code, a missing annotation, not a disagreement about advertised semantics; both annotators independently flagged the underlying defect in their own notes. The full grammar, clause-authoring protocol, segmentation rules, and worked-example detail are in the artifact.

**AI-use disclosure.** Both annotator sets above, and the numeric analyses of §IV, §VI, and §VII, were produced by agent-run scripts and sessions, disclosed here per IEEE's policy on generated content.
