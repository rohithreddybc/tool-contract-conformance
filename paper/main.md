...
Every contract clause cites the provenance of the advertised surface it operationalizes — file, line, and a verbatim quote checked against the pinned commit at build time — across three grounding tiers: **agent-visible** text (docstring, schema, prompt template, tool return, README, external standard) the agent was actually conditioned on; **maintainer-annotated** text (a comment, log line, or TODO) stating intended semantics no agent ever sees; and **inferred** semantics we assigned ourselves. Only agent-visible clauses are headline-eligible — a violation there corrupts the measurement by §III's argument — while the other two tiers are reported separately and never folded into a headline count; this is the paper's construct-validity firewall against the objection that our contracts are just our opinions. A simplification is benign exactly when it is advertised: the defect is never the simplification but the undisclosed divergence between what the interface tells the agent and what the implementation does, and either side of a divergence may be repaired, a documentation fix resolving a finding exactly as completely as a code fix (§VIII). The checker also knows when not to fire: tau2-bench's advertised deferred flight-database update and its advertised non-enforcement of cancellation eligibility are both true negatives our test suite exercises, the credibility anchor behind every VIOLATES verdict elsewhere in the paper. Contract authoring is measured, not trusted: two independently authored annotator sets are compared extensionally against a shared, pre-committed probe corpus across six tools. The blind stratum — five tools annotated with no knowledge of which were defective — agrees on 98 of 98 probe comparisons across 16 clause pairs; a sixth tool, `cancel_reservation`, forms a contaminated stratum (Annotator B had read a worked example of this exact tool beforehand) and is reported separately, never pooled with the blind result. One finding flips within the blind stratum: `reserve_car_rental`'s `arg.end_time` is VIOLATES under Annotator A and UNTESTABLE under B, because A's contract declares a `probe_values` hint pinning the argument to parseable ISO strings and B's does not, so every probe B's contract can build fails inside `datetime.fromisoformat()` before reaching the tool's effect code — a missing annotation, not a disagreement about advertised semantics; both annotators independently flagged the underlying defect in their own notes. The full grammar, clause-authoring protocol, segmentation rules, and worked-example detail are in the artifact.

---

## VI. The Conformance Checker

The checker has a static half — five AST-only checks that flag candidate sites for a person to read — and a dynamic half that produces verdicts: snapshot a fresh environment, invoke the tool, snapshot again, and evaluate every clause against the resulting `(pre, post, args, result)` tuple, resolving to CONFORMS, VIOLATES with a minimal witness, or UNTESTABLE with a reason code; UNTESTABLE stays in every denominator this paper reports. tau2-bench, AgentDojo, and MM-ToolSandbox run through adapters as subprocesses; MedAgentBench has no adapter and is checked statically (§I). All six defect classes now have a real dynamic or static code path, but four of them did not until a post-freeze mutation sweep exposed the gap — Reset Leak and Invariant Break verdicts, 35 frame clauses, and 17 of 19 tau2 contracts were silently never dynamically exercised — and all four are fixed, at the cost of one disclosed refreeze cycle (`checker-freeze-v1` to `checker-freeze-v2`) rather than two. Adapter mechanics, the subprocess boundary, and the remaining frame-path expressiveness gap are in the artifact.
