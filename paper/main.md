...
Autonomous agents that call external tools are moving into consequential settings, including clinical decision support, where a wrong action carries a real cost. The benchmarks that measure whether an agent is ready for that responsibility score it by executing tool calls against a simulated environment and reading the resulting state as trustworthy: existing evaluation checks that a tool reported success, not that the state it describes actually changed. None of the audits we survey test that assumption; they inspect tasks, gold solutions, and graders, never the tool implementation, so a benchmark whose tool silently no-ops can report the same success as one that behaves correctly, and every downstream leaderboard and model-selection decision built on that score inherits the corruption, silently and reproducibly, on every rerun. We close this gap by authoring executable contracts from each tool's advertised surfaces (docstring, schema, prompt text, return value), checking implementations and state transitions against those contracts with a static and dynamic checker, and tracing which task verdicts depend on state a defective tool should have written. Across four benchmarks we confirm four headline-eligible tool-layer defect cells, from eight verified instances: three demonstrated dynamically and one, MedAgentBench's Phantom Effect, statically. Its write path is a no-op; no grader reads the record back, and where a write oracle exists at all it reconstructs the write from the agent's own transcript, gated on a fabricated success string. Score-at-risk bounds are basis-tagged rather than pooled: an exact-field evaluator puts 1,135 of 2,285 tasks in tau2's full generated telecom file (not its own smaller published-evaluation subset) at risk, while a coarser whole-database-hash basis puts all 50 airline tasks at risk by construction. Detection on the mutation arm is precise (≥0.867, zero false positives over 25 flags) but recall is weak, likely underpowered, at design-effect-adjusted Wilson lower bounds on real tools; an open-world mutation arm gives an escape rate ≥0.956 on a toy domain and an undefined rate, zero live mutants, on tau2. A pre-registered (scope: §IX) trajectory-replay check on ten gold trajectories exercises the defect zero times and records zero verdict flips.
...
The loss such stubbing can cause is not hypothetical. MedAgentBench [medagentbench25] evaluates language-model agents against a simulated electronic health record. Its harness handles every write request an agent emits in one branch of one function (`src/server/tasks/medagentbench/__init__.py`, lines 85–91 at commit `9926011`, abridged):
...
The checker's demonstrated role here is confirmation and score-tracing, not discovery: §VIII's Surfaced-by column shows seven of eight findings surfaced manually, one by static check, and its contribution is to reproduce three of the four headline cells under contract and trace their reach into published verdicts, not to have found them first. We did not invent design by contract, contract inference, or benchmark auditing either: §II concedes the technique and rests our novelty on the target and the consequence for published scores. One result shapes the whole argument regardless: MedAgentBench's grader admits evidence only when followed by the literal success string its own defective tool fabricates, so tool and grader are mutually consistent, and five prior audits reported nothing (§II, §VIII), the same cross-layer blindness this paper is built to catch. The rest of this paper proceeds as follows: §II surveys prior benchmark grading, completion-verification, and contract-testing work; §III fixes the notation and the defect taxonomy; §IV traces score-at-risk dependencies; §V and §VI present the contract specification and the conformance checker; §VII validates detection; §VIII reports the audit findings and coordinated disclosure; §IX examines threats to validity; and §X concludes.
...
A benchmark tool is not application code that happens to be simulated; it is the write path of a measuring device, and a defective one corrupts a published number silently and reproducibly rather than filing an issue, with the corrupted number still cited either way. This section fixes the object under test, the defect classes, and the principle that separates a defect from a legitimate simplification.
...
**MedAgentBench (Findings 1 and 4) is the paper's central chain.** Finding 1 is the no-op POST branch quoted in §I. Finding 4 makes it undetectable: the grading module reconstructs each write from the transcript, admitting evidence only when it matches Finding 1's fabricated success string (§II). No grader ever issues a FHIR read to verify the write. Patching the tool to perform real writes would change no score, because no grader observes the state a patch would repair — why §VII attempts no A/B experiment here. Finding 4's evidence file, `refsol.py`, is not distributed with the repository; MedAgentBench's own README directs a reader to an unhashed, unversioned Stanford Medicine Box share for it. We pin a copy in the artifact at its SHA-256 hash (`repro/env/refsol.py`) so a reader can verify §IV's 60/90/150/0 classification against it, not our transcription.
...
**Ranked by how much each could move the conclusions above, rather than left at flat salience:** the missing recorded-real-calls corpus first, since it alone explains the tau2 zero denominator, the agreement flip, and 29 of 33 misses; whether an exception absorbed at the subprocess boundary scores as a detection or a silent non-detection second, since it bounds what every recall number in this paper means; and the anchor-driven benchmark selection third, since it forecloses any prevalence claim. The other seven limitations above stand as stated; this ranking reorders none of them, it only says which three to read first.
