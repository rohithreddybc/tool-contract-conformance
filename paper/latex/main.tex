...
One result shapes the whole argument: MedAgentBench's grader for write tasks reconstructs the write from the agent's own transcript, gated on the literal success string the harness fabricates, so the defective tool and the grader that should catch it are mutually consistent --- each layer looks correct audited alone, which is why five prior audits reported nothing (\S\,II, \S\,IX).
...
The checker did not arrive at that state complete: a mutation-corpus sweep run after \texttt{checker-freeze-v1} found four gaps the taxonomy had been claiming as closed, all fixed before any number in \S\,VII was scored, at the cost of one disclosed refreeze cycle (\S\,VII, \S\,X). One expressiveness gap is left open rather than patched --- the frame-path grammar cannot exclude a single key from a wildcard match --- and is stated in full as a limitation in \S\,X rather than patched around here.
...
These bounds do not indict the taxonomy on their own. \texttt{experiments/decompose\_closed\_world\_misses.py} traces the cause of all 33 real-tool misses: 29 are \texttt{probe\_corpus\_unreachable} (the frozen corpus never drove the mutated path), 2 are a genuine \texttt{clause\_gap}, 1 a \texttt{probe\_gap}, 1 \texttt{target\_not\_found}. Per operator, M-PRECOND's 6 misses and M-INVAR's 8 are all unreachable; M-IGNARG's 12 are 9 unreachable, 2 clause gaps, 1 target-not-found; M-PARTIAL's 7 are 6 unreachable, 1 probe gap. M-INVAR, decomposed by liveness screening only (\texttt{operators\_liveness\_only}) not the full three-way classifier, has a 0.000 that reads as ``no probe ever reached a mutated invariant path,'' not ``the checker cannot detect invariant breaks'' --- only 2 of 33 misses are a taxonomy gap the checker itself owns. Two questions stay open: whether closed-world mutants were screened for behavioural liveness before entering this denominator, and whether the six watchdog timeouts (M-PHANTOM 2, M-IGNARG 4) are exchangeable with misses for the bounds above. The bounds are unchanged; only what a miss means changes. M-RESET has no real-tool site to draw at all (\S\,X). Biconditional success, the mechanism M-PHANTOM's detector depends on, is opt-in per contract and is adopted on 18 of 19 tau2-bench contracts, 1 of 7 AgentDojo contracts, and 0 of 5 MM-ToolSandbox contracts; where it is not adopted, Phantom Effect cannot fire on that tool at all, a second, more visible reason M-PHANTOM's own recall sits at 0.359 rather than higher.
...
\textbf{One refreeze cycle.} The table above is scored under \texttt{checker-freeze-v2} (commit \texttt{5824376}), not the first freeze: a mutation sweep against \texttt{checker-freeze-v1} found four claims the taxonomy had been carrying as closed with no code behind them --- Reset Leak and Invariant Break had no code path that ever produced their verdict, thirty-five shipped frame clauses were never evaluated against a real snapshot pair, and a hand-written routing table left seventeen of nineteen tau2-bench contracts never dynamically exercised though they validated and counted everywhere else. All four are fixed as of the second freeze, at the cost of one disclosed refreeze cycle (\S\,VI, \S\,X).
...
\textbf{The agent-experiment sample is provisional at N=5 per finding (\S\,VIII), and a null at that size proves less than it looks like it proves.} F3's affected population of 7 tasks makes 5 close to exhaustive; F2's population of 1,120 does not, so zero of ten trajectories exercising either trigger predicate cannot separate a defect latent under every gold trajectory from one an undersized sample simply missed.
...
\textbf{One refreeze cycle is disclosed rather than folded into the final numbers (\S\,VI, \S\,VII).} A mutation-corpus sweep against \texttt{checker-freeze-v1} found four taxonomy claims carried as closed with no code behind them; all four were fixed and the checker refrozen as \texttt{checker-freeze-v2} before any number in \S\,VII was scored. The cost of finding them --- one refreeze cycle rather than zero --- is stated plainly rather than absorbed into the total.
